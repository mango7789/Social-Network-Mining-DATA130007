...
def main():
    ...
    parser.add_argument('--proximity', default='first-order', help='Proximity type: first-order or second-order')
    ...
    parser.add_argument('--is_all', action='store_true', help='Train with all edges; no validation or test set')
...
def train(args):
    ...
    for b in range(args.num_batches):
        ...
        # 前向传播
        if model_type == 'lace':
            similarity, embedding, context_embedding = model(u_i, u_j, proximity=args.proximity)
            ...
            if model.val_set and ((b + 1) % 50 == 0 or b == args.num_batches - 1):
                with torch.no_grad():
                    ...
                    if model_type == 'lace':
                        ...
                        val_energy = model.energy_kl(val_u_i, val_u_j, proximity=args.proximity)
                    ...
                    if model_type == 'glace':
                        ...
                        val_auc, val_ap = score_link_prediction(data_loader.val_ground_truth, val_energy.cpu().numpy())
                    model.train()
                print(f"{b}\t{loss.item():.6f}\t{val_auc:.6f}\t{val_ap:.6f}\t{sampling_time_total:.2f}\t{training_time_total:.2f}\t{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
            else:
                print(f"{b}\t{loss.item():.6f}\t{sampling_time_total:.2f}\t{training_time_total:.2f}\t{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
        ...
        if (b + 1) % 50 == 0 or b == args.num_batches - 1:
            with torch.no_grad():
                ...
                if model_type == 'glace':
                    ...
                    kl_distance_all, mu_all, sigma_all, _ = model(all_nodes, all_nodes, proximity=args.proximity)
                    ...
                    with open(f'emb/{model_type}_{args.name}_embedding_{suffix}.pkl', 'wb') as f:
                        pickle.dump(embedding_data, f)
                else:
                    # 对于LACE模型，保存embedding
                    all_nodes = torch.arange(args.X.shape[0]).long().to(device)
                    similarity_all, embedding_all, _ = model(all_nodes, all_nodes, proximity=args.proximity)

...
def move_args_to_device(args, device):
    ...
    for key, value in vars(args).items():
        # 如果是 PyTorch 张量，直接将其移动到指定设备
        if isinstance(value, torch.Tensor):
            setattr(args, key, value.to(device))
        # 如果是 SciPy 稀疏矩阵，转换为 PyTorch 稀疏张量
        elif isinstance(value, sp.spmatrix):
            # 将 csr_matrix 转换为 PyTorch 稀疏张量
            if isinstance(value, sp.csr_matrix):
                ...
                # 创建 PyTorch 稀疏张量并移动到指定设备
                sparse_tensor = torch.sparse_coo_tensor(
                    torch.stack([row_indices, col_indices]), values, size
                ).to(device)
                setattr(args, key, sparse_tensor)

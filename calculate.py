...
def compute_community_metrics(
    node: pd.DataFrame, edge: pd.DataFrame, community_file: str, output_json: str
) -> None:
    """
    ...
    G.add_edges(
        [(src, dst) for src, dst in zip(edge["src"], edge["dst"]) if src != dst]
    )

    G.vs["name"] = node["id"].tolist()

    # 将社区标签映射到图的节点
    community_mapping = dict(zip(community_labels["id"], community_labels["community"]))

    G.vs["community"] = [
        community_mapping.get(node_id, None) for node_id in G.vs["name"]
    ]

    # 处理没有社区标签的节点
    missing_community_nodes = [v["name"] for v in G.vs if v["community"] is None]
    ...
    largest_community_ids = [community_id for community_id, size in largest_communities]

    print(f"最大的10个社区的社区ID: {largest_community_ids}")

    # 使用并行处理来加速社区计算
    def calculate_community_metrics(community_id):
        community_nodes = [v.index for v in G.vs if v["community"] == community_id]
    ...
    with ProcessPoolExecutor(max_workers=8) as executor:  # 调整工作进程数
        results = list(
            tqdm(
                executor.map(calculate_community_metrics, largest_community_ids),
...
if __name__ == "__main__":
    ...
    compute_community_metrics(node_data, edge_data, community_file, output_json)
